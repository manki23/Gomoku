...
{
    std::unordered_set<std::string> fiveInRowSet({"XXXXXXX", "_XXXXX_", "OXXXXXO", "_XXXXXO", "XXXXXXO", "OXXXXXX", "OXXXXX_", "_XXXXXX", "XXXXXX_"});
    std::unordered_set<std::string> liveFourSet({"X_XXXXX", "_X_XXXO", "OX_XXXX", "XXX_X_X", "_OXX_XX", "XX_XXO_", "X_XX_XX", "XOX_XXX", "_OX_XXX",
    ...
    std::unordered_set<std::string> deadFourSet({"OXXXX__", "OXXXX_O", "OXXXX_X", "_OXXXX_", "OOXXXX_", "XOXXXX_"});
    std::unordered_set<std::string> deadThreeSet({"OXXX__X", "OXXX__O", "OXXX___", "_OXXX__", "OOXXX__", "XOXXX__", "___XXXO", "X__XXXO", "O__XXXO",
    ...
    std::unordered_set<std::string> liveThreeSet({"_O_XXX_", "OOXX_X_", "XX_XX_O", "_OX_XX_", "_X_X_X_", "_X_XXOO", "__XX__X", "_X_XXO_", "__X_XXO",
    ...
    std::unordered_set<std::string> deadTwoSet({"OX__X__", "OX__X_O", "OX__X_X", "_OX__X_", "OOX__X_", "XOX__X_", "_X__XO_", "_X__XOO", "_X__XOX",
    ...
    std::unordered_set<std::string> liveTwoSet({"_X___X_", "_X___XO", "_X___XX", "OX___X_", "OX___XO", "OX___XX", "XX___X_", "XX___XO", "XX___XX",
    ...
    std::unordered_set<std::string> uselessOneSet({"___X___", "___X__O", "___X__X", "O__X___", "O__X__O", "O__X__X", "X__X___", "X__X__O", "X__X__X",
    ...
    for (std::vector<std::string>::const_iterator it = patterns.begin();
        it != patterns.end(); it++)
    {
        if (inSetPattern(*it, fiveInRowSet))
            patternMapScore["fiveInRow"] += 1;
        else if (inSetPattern(*it, deadFourSet))
            patternMapScore["deadFour"] += 1;
        else if (inSetPattern(*it, liveFourSet))
            patternMapScore["liveFour"] += 1;
        else if (inSetPattern(*it, deadThreeSet))
            patternMapScore["deadThree"] += 1;
        else if (inSetPattern(*it, liveThreeSet))
            patternMapScore["liveThree"] += 1;
        else if (inSetPattern(*it, deadTwoSet))
            patternMapScore["deadTwo"] += 1;
        else if (inSetPattern(*it, liveTwoSet))
            patternMapScore["liveTwo"] += 1;
        else if (inSetPattern(*it, uselessOneSet))
...
std::map<std::string, int > getPatternDict(Game * g, int player, int opponent)
{
    ...
    for (int y = 0; y < g->goban_size; y++)
    {
        for (int x = 0; x < g->goban_size; x++)
        {
            ...
            {
                //## get line pattern
                if (g->board[y][x + i] == player)
                    line[i-low] = 'X';
                else if (g->board[y][x + i] == opponent)
                    line[i-low] = 'O';
                else if (isOutsideBoard(x + i, y, g->goban_size, g->forbidden_moves[player]))
                    line[i-low] = '.';

                //## get column pattern
                if (g->board[y + i][x] == player)
                    column[i-low] = 'X';
                else if (g->board[y + i][x] == opponent)
                    column[i-low] = 'O';
                else if (isOutsideBoard(x, y + i, g->goban_size, g->forbidden_moves[player]))
                    column[i-low] = '.';
                
                //## get left diagonal pattern
                if (g->board[y + i][x + i] == player)
                    leftDiag[i-low] = 'X';
                else if (g->board[y + i][x + i] == opponent)
                    leftDiag[i-low] = 'O';
